...
#define toSizeT(t)     (((t-1)/SIZE_T)+1)*SIZE_T  // TODO
//
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      if(toSizeT(t)>getRemainSize()) return false;
      ret=(T*)_ptr;
      _ptr=_ptr+toSizeT(t);
      return true;
   }
...
private:
   ...
   T* getMem(size_t t) {
      ...
      size_t mem = toSizeT(t);

      if(mem>_blockSize)
      {
         cerr << "Requested memory (" << mem << ") is greater than block size"
              << "(" << _blockSize << "). " << "Exception raised...\n";
         throw bad_alloc();    
      }

     
      if(getMemRecycleList(getArraySize(mem))->numElm()!=0)
      {
         ret=getMemRecycleList(getArraySize(mem))->popFront();

         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << getArraySize(mem) << "]..." << ret << endl;
         #endif //MEM_DEBUG
      }
      else
      {
         if(_activeBlock->getMem(mem,ret)!=0)
         {
            return ret;
         }
         else
         {
            ...
            {  
               getMemRecycleList(getArraySize(downtoSizeT(_activeBlock->getRemainSize())))->pushFront((T*)_activeBlock->_ptr);
               #ifdef MEM_DEBUG
               cout << "Recycling " << ret << " to _recycleList[" << getArraySize(downtoSizeT(_activeBlock->getRemainSize())) << "]\n";
               #endif // MEM_DEBUG
