      ...
      if(_totallist[i]!=NULL)
      {
        ...
        if(_totallist[i]->_faninID.size()!=0)
        {
          while(temp<2)
          {
            ...
            if(gate!=NULL)
            {
              CirGateV gate1 = CirGateV(gate,(_totallist[i]->_faninID[temp])%2); 
              CirGateV gate2 = CirGateV(_totallist[i],(_totallist[i]->_faninID[temp])%2);
              _totallist[i]->_fanin.push_back(gate1);
              gate->_fanout.push_back(gate2);
            }
            else
            {
...
CirMgr::printFECPairs() const
{
 for(size_t i=0; i < _FecGrps.size(); i++)
 {
  ...
  {
    if(_totallist[_FecGrps[i][j]]->_sim==(_totallist[_FecGrps[i][0]]->_sim))
    {
      cout<<" "<<_FecGrps[i][j];
    }
    else
...
CirMgr::genDFS()
{
    dfslist.clear();
    booldfs.clear();
    booldfs.resize(_totallist.size(),0);
    CirGate::setGlobalRef();
    for(unsigned i=0;i<_polist.size();i++)
    {
      _totallist[_polist[i]]->dfsTraversal(dfslist,booldfs);
    }
    unsigned temp=0;
...
{
   unsigned temp=0;
   cout << "PIs of the circuit:"<<endl;
   while(temp!=_pilist.size())
   {
    cout <<_totallist[_pilist[temp]]->getTypeStr()<<endl;
    cout << " " <<_pilist[temp]<<endl;
    temp++;
   }
   cout << endl;
...
CirMgr::writeAag(ostream& outfile) const
{
 ...
 for(unsigned i=0;i<_polist.size();i++)
 {
  unsigned id = _totallist[_polist[i]]->_fanin[0].gate()->getID();
  id = id*2+(_totallist[_polist[i]]->_fanin[0].isInv()?1:0);
  outfile<<id<<endl;
 }
 for(unsigned i=0;i<dfslist.size();i++)
 {
  if(dfslist[i]->getTypeStr()=="AIG")
  {
    unsigned id1 = dfslist[i]->_fanin[0].gate()->getID();
    unsigned id2 = dfslist[i]->_fanin[1].gate()->getID();
    id1 = id1*2+(dfslist[i]->_fanin[0].isInv()?1:0);
    id2 = id2*2+(dfslist[i]->_fanin[1].isInv()?1:0);
    outfile<<(dfslist[i]->getID())*2<<" "<<id1<<" "<<id2<<endl;
  }
 }
