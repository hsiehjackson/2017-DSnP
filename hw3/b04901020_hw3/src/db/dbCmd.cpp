...
DBAppendCmd::exec(const string& option)
{
   ...
   if(options.empty())
   {
      return CmdExec::errorOption(CMD_OPT_MISSING,temp);
   }
   ...
   if (doRow) {   
      ...
      if (options.size()<dbtbl.nCols())
      {
         for(size_t i=options.size();i<dbtbl.nCols();i++)
         {
            options.push_back("-");
         }
      }
      ...
      for(size_t i=0;i<dbtbl.nCols();i++)
      {
         ...
         else if(myStr2Int(options[i],temp)==0)
         {
            cerr << "Error:\""<<options[i]<<"\" is not an integer!!";
            return CMD_EXEC_ERROR;
         }
         else if(myStr2Int(options[i],temp)==1)
         {
            num.push_back(temp);
         }
      }
      ...
      if (options.size()<dbtbl.nRows())
      {
         for(size_t i=options.size();i<dbtbl.nRows();i++)
         {
            options.push_back("-");
         }
      }
...
{  
   ...
   int r,c;
   int sw; 
        if (myStrNCmp("-Row", options[0], 2) == 0) sw=1; 
   else if (myStrNCmp("-Column", options[0], 2) == 0) sw=2;
   else if (myStrNCmp("-Table",  options[0], 2) == 0) sw=3;
   else if (myStrNCmp("-Summary", options[0], 2) == 0) sw=4;
   else if (!checkRowIdx(options[0], r)) return CMD_EXEC_ERROR;
   else if (checkRowIdx(options[0], r)) sw=0;
